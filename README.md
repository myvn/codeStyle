...
```bash
npm run test:all               # 全量运行 180 项：用例明细 + 分类统计 + 最慢文件定位
...
同机复测（180 项，同一台 2 核沙箱连续跑；绝对秒数随机器负载波动，相对关系稳定）：串行套件 + 文件级并发 1 = 72.7s、默认自动并发 3 = 70.3s、`--parallel` 四套件 = 71.1s、`npm run test:integration` = 43.1s、`node --test demo-test/integration/*.test.cjs` = 71.6s。
