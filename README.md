...
```text
  ...
  套件                      通过    失败    跳过    用时
  ──────────────────────────────────────────────────────
  基础 CLI 与配置矩阵         66       0       -    4.6s
  现代工具链与提交链          54       0       -   64.4s
  ESLint 8 兼容性             27       0       -   17.7s
  ──────────────────────────────────────────────────────
  合计                       147       0       -   86.7s
  ──────────────────────────────────────────────────────

  ✅ 全部通过：147/147 项，用时 86.7s
```
...
两层并行的实现：集成套件按"重载用例各自成文件"拆成 11 个文件，`node --test` 默认按
CPU 核数并行跑这些文件（文件级），套件之间再由运行器并行（套件级）。因为每个文件里
跑的都是真实 `git commit`（单次 1.7–4.2s），天花板由**最慢的那个文件**决定：

| integration 子文件 | 单独耗时（2 核） | 内容 |
| ------------------ | ---------------- | ---- |
| `commit-chain-{scss,less,both}` | 6.2–6.8s | 三种预处理器的完整提交链 |
| `embedded-style` | 6.3s | Vue/nvue 内嵌样式拦截 |
| `partial-staging` | 6.2s | 部分暂存与冲突恢复 |
| `error-recovery` / `-style` | 4.8s / 3.9s | 不可修复错误的回滚 |
| `git-edge` / `git-mv-rm` | 4.3s / 4.2s | 首次提交、stash、git mv/rm |
| `lint` | 4.1s | 规则与格式化行为 |
| `release` | 0.8s | standard-version |

两层叠加后的实测（2 核沙箱，147 项）：

| 运行方式                              | 墙钟时间 |
| ------------------------------------- | -------- |
| 串行套件 + 文件级并发 1（CI 默认形态） | 74s      |
| 串行套件 + 文件级并发 11              | 58.6s    |
| `--parallel` + 文件级并发 11（16 核默认） | 55.6s |
| 单独跑集成套件 + 文件级并发 11        | 43.0s    |

（2 核沙箱里并行收益被 CPU 争抢吃掉大半；16 核机器上集成套件那段 ≈ 最慢子文件，
2 核上最慢子文件是 6.8s，多核机器会显著更短。）
