...
| 运行方式                              | 墙钟时间 |
| ------------------------------------- | -------- |
| 串行套件 + 文件级并发 1（CI 默认形态） | 90s      |
| 串行套件 + 文件级并发 4               | 74.6s    |
| `--parallel` + 文件级并发 4           | 68.5s    |
| 单独跑集成套件 + 文件级并发 4         | 51.9s    |

核数越多收益越大：集成套件单文件耗时 `lint 5.7s / commit-chain 26.5s /
error-recovery 19.0s / git-edge 20.0s`，多核并行下这段的上限是其中最慢的 26.5s。
