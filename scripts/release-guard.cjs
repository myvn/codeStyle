 ...
 *
 * 为什么需要它：
 * commit-and-tag-version（standard-version 的维护分支）在「HEAD 已经打过 v* tag、之后没有任何新提交」时不会报错，而是把
 * 版本号再抬一级、写出一段没有条目的空 CHANGELOG，然后照常提交 + 打 tag。实测
 * （standard-version 9.5.0 实测，catv 行为相同，tag 打在 HEAD 上）：1.0.0 → 1.0.1，CHANGELOG 只有标题行，
...
if (dirty.length > 0) {
    ...
    abort([
        ...
        ...shown,
        "  发布工具会把它们一起卷进版本提交里，请先 `git commit` 或 `git stash`。",
    ])
}
...
if (tagsAtHead.length > 0) {
    abort([
        ...
        "  此时再跑发布工具不会报错，而是把版本号再抬一级、写出一个没有条目的空 CHANGELOG，",
