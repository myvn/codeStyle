...
async function runOneFile(suiteState, file, liveCases) {
    ...
    if (!liveCases && !QUIET && !VERBOSE) {
        console.log(
            `      ▸ ${short} · ${fileState.passed + fileState.failed + fileState.skipped} 项 · ${seconds(duration)}`,
        )
        for (const item of fileState.cases) {
            console.log(caseLine(item))
        }
    }
    if (VERBOSE) {
...
function printProfile(state) {
    if (!PROFILE || state.files.length < 2) {
        return
    }
...
function clearBoard() {
    if (!PROGRESS || VERBOSE || !board.printed) {
        return
    }
    process.stdout.write(`\x1b[${board.printed}F`)
    for (let i = 0; i < board.printed; i += 1) {
...
async function main() {
    ...
    const fileInfo = `${FILE_JOBS} 文件并发${
        FILE_JOBS_AUTO ? `（自动：${CORES} 核 / 内存 ${TOTAL_MEM_GB.toFixed(0)}G）` : ""
    }`
    ...
    if (PARALLEL) {
        ...
        const timer = PROGRESS && !VERBOSE ? setInterval(() => renderBoard(states), 1000) : null
        if (timer?.unref) {
            timer.unref()
        }
        renderBoard(states)
    ...
    const allFailures = results.flatMap((r) =>
        r.failures.map((f) => ({ suite: r.suite.title, ...f })),
    )
    if (allFailures.length) {
        ...
        for (const failure of allFailures) {
            ...
            for (const line of failure.lines.slice(0, 30)) {
                console.log(`    ${line}`)
            }
            if (failure.lines.length > 30) {
                console.log(`    …（其余 ${failure.lines.length - 30} 行省略）`)
            }
        }
        console.log("")
    }
    ...
    if (slowest.length > 1) {
        console.log(
            `  ⏱ 最慢文件：${slowest.map((f) => `${f.name} ${seconds(f.duration)}`).join(" · ")}`,
        )
        console.log("")
    }
    ...
    process.exitCode =
        totals.failed > 0 || skippedSuites.length > 0 || crashedSuites.length > 0 ? 1 : 0
}
