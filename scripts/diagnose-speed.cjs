...
function gitBaseline(baseDir, label, options = {}) {
    ...
    const elapsed = best(() => {
        run("git", ["commit", "-q", "--allow-empty", "-m", "chore: baseline"], {
            cwd: dir,
            ...options,
        })
        run("git", ["commit", "-q", "--allow-empty", "-m", "chore: second"], {
            cwd: dir,
            ...options,
        })
    })
...
if (fs.existsSync(RUNTIME)) {
    gitBaseline(RUNTIME, "仓库内 · 隔离 ~/.gitconfig", isolate)
}
...
if (fs.existsSync(RUNTIME)) {
    gitIndexOps(RUNTIME, "仓库内 · 隔离 ~/.gitconfig", isolate)
}
console.log("")
...
if (fs.existsSync(RUNTIME)) {
    writeFiles(RUNTIME, "仓库内（demo-test/.runtime）")
}
console.log("")
...
if (SKIP_CHAIN) {
    ...
    try {
        ...
        line(
            "建仓 + bin/init + husky 安装",
            `${ms(process.hrtime.bigint() - started).toFixed(0)}ms`,
        )
        ...
        line(
            "第 3 次提交（--no-verify，无 hooks）",
            `${noHookMs.toFixed(0)}ms`,
            "对照：纯 git 成本",
        )
        ...
        console.log(
            "        它由 npx（npm CLI）启动 + lint-staged + prettier/eslint/stylelint 组成；",
        )
...
async function stress() {
    ...
    if (soloFile) {
        line("1× 完整链（含 hooks，同一测试文件）", `单 ${(soloFile / 1000).toFixed(1)}s`)
    }
    console.log("")
    ...
    const cases = [
        ...
        [
            "npx --no-install lint-staged --version",
            "npx",
            ["--no-install", "lint-staged", "--version"],
        ],
    ]
    ...
    if (fs.existsSync(sample)) {
        for (const [label, options] of [
            ...
            [
                "同一个测试文件（fixture 在 TMPDIR）",
                { env: { MY_CODE_STYLE_FIXTURE_DIR: os.tmpdir() } },
            ],
        ...
        console.log(
            "        纯 git 链膨胀小、完整链膨胀大 → 排队在 hooks（eslint/prettier/stylelint）里；",
        )
