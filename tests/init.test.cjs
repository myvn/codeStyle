...
function fixture(t, files = {}) {
    ...
    return {
        dir,
        run: (...args) => spawnSync(process.execPath, [cli, ...args], { cwd: dir, encoding: "utf8" }),
...
function snapshot(dir) {
    return fs.readdirSync(dir).sort().map((name) => {
        ...
        return [name, fs.statSync(file).isDirectory() ? snapshot(file) : fs.readFileSync(file, "utf8")]
    })
}
const manifest = (version = "^9.0.0", extra = {}) => JSON.stringify({
    name: "fixture", devDependencies: { eslint: version }, ...extra,
})
for (const invalid of ["{invalid", "null", "[]", '{"dependencies":{"eslint":9}}', '{"scripts":[]}']) {
    test(`invalid manifest is read-only: ${invalid}`, (t) => {
        const f = fixture(t, { "package.json": invalid, ".prettierrc.cjs": "custom", ".husky/pre-commit": "custom hook" })
        const before = f.snapshot()
        assert.equal(f.run().status, 1)
        assert.deepEqual(f.snapshot(), before)
    })
}
for (const args of [["--help"], ["-h"], ["--version"], ["-v"], ["--dry-rnu"], ["--version", "--bad"]]) {
...
for (const name of ["eslint.config.js", "eslint.config.mjs", "eslint.config.cjs", "eslint.config.ts"]) {
    test(`preserve existing Flat Config: ${name}`, (t) => {
        ...
        assert.deepEqual(fs.readdirSync(f.dir).filter((n) => n.startsWith("eslint.config.")), [name])
    })
}
for (const [version, name] of [["^9", ".eslintrc.cjs"], ["^9", ".eslintrc.json"], ["^8", "eslint.config.js"]]) {
...
for (const [version, name] of [["^9", "eslint.config.mjs"], ["^8", ".eslintrc.cjs"]]) {
    test(`fresh initialization and dry-run: ${version}`, (t) => {
        const f = fixture(t, { "package.json": manifest(version, { scripts: { lint: "custom lint" } }) })
        const before = f.snapshot()
