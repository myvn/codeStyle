...
for (const scenario of cases) {
    test(`初始化场景：${scenario.name}`, (t) => {
        const p = project(t, {
            "package.json": JSON.stringify({ name: "demo-consumer", devDependencies: scenario.dependencies }),
        })
        const result = p.init()
        ...
        assert.equal(pkg.scripts.lint, scenario.legacy ? "eslint . --ext .js,.cjs,.mjs,.ts,.mts,.cts" : "eslint .")
        assert.equal(pkg.scripts.prepare, "husky")
        const tasks = pkg["lint-staged"]
        assert.equal(Object.values(tasks).flat().includes("stylelint --fix"), Boolean(scenario.style))
        if (scenario.style) {
            const ext = scenario.style.endsWith("/less") ? "less" : "scss"
            assert.deepEqual(tasks[`**/*.{html,css,${ext}}`], ["prettier --write", "stylelint --fix"])
        }
        ...
        for (const file of [".prettierrc.cjs", ".prettierignore", ".editorconfig", ".gitattributes", ".gitignore", ".versionrc.js"]) {
...
test("已有 scripts 和 gitignore 保留；lint-staged 按现有设计替换", (t) => {
    const p = project(t, {
        "package.json": JSON.stringify({
            name: "demo-consumer", scripts: { lint: "custom-lint", build: "custom-build" },
...
test("项目类型可从 peerDependencies 检测", (t) => {
    const p = project(t, {
        "package.json": JSON.stringify({ name: "demo-consumer", peerDependencies: { vue: "^3", eslint: "^9" } }),
    })
    assert.equal(p.init().status, 0)
...
test("Less 缺失依赖提示应包含 stylelint-config-html", (t) => {
    const p = project(t, {
        "package.json": JSON.stringify({ name: "demo-consumer", devDependencies: { vue: "^3", less: "^4" } }),
    })
    const result = p.init()
