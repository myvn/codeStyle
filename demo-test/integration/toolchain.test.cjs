...
assert.ok(fs.existsSync(path.join(runtime, "node_modules/eslint")), "先运行 npm run test:integration:setup；安装冲突时见 demo-test/README.md")
...
async function eslint(level = "base") {
    const config = (await import(pathToFileURL(path.join(copy, `src/eslint/flat/${level}.mjs`)).href)).default
...
test("ESLint 报出格式错误并修复；再次检查无错误", async () => {
    const config = (await import(pathToFileURL(path.join(copy, "src/eslint/flat/base.mjs")).href)).default
    ...
    const fixer = new ESLint({ cwd: runtime, overrideConfigFile: true, overrideConfig: config, fix: true })
...
for (const [entry, ext, source] of [
    ...
    test(`Stylelint 检查和修复：${ext}`, async () => {
        ...
        const options = { config, configBasedir: runtime, codeFilename: path.join(runtime, `demo.${ext}`) }
        ...
        assert.ok(bad.results.some((result) => result.warnings.some((warning) => warning.rule === "property-no-unknown")))
    })
}

async function commitlint(message) {
    const load = (await import(pathToFileURL(localRequire.resolve("@commitlint/load")).href)).default
    const lint = (await import(pathToFileURL(localRequire.resolve("@commitlint/lint")).href)).default
    const config = await load(localRequire("my-code-style/commitlint"), { cwd: runtime, file: path.join(copy, "src/commitlint/base.cjs") })
    return lint(message, config.rules, { parserOpts: config.parserPreset?.parserOpts, ignores: config.ignores, defaultIgnores: config.defaultIgnores })
}
...
test("生成的 commit-msg hook 真正阻止非法提交（仅临时 Git 仓库）", (t) => {
    ...
    function run(command, args, env = {}) {
        return spawnSync(command, args, { cwd: dir, encoding: "utf8", timeout: 15000, env: { ...process.env, ...env } })
    }
    function git(...args) {
    ...
    fs.writeFileSync(path.join(dir, "package.json"), '{"name":"hook-fixture","devDependencies":{"eslint":"^9"}}')
    ...
    const husky = run(process.execPath, [path.join(path.dirname(localRequire.resolve("husky")), "bin.js")])
    ...
    const rejected = run("git", ["-c", "commit.gpgsign=false", "commit", "--allow-empty", "-m", "invalid: broken message"])
    assert.notEqual(rejected.status, 0)
    assert.match(rejected.stdout + rejected.stderr, /type-enum/)
    const accepted = run("git", ["-c", "commit.gpgsign=false", "commit", "--allow-empty", "-m", "feat: add demo"])
    assert.equal(accepted.status, 0, accepted.stdout + accepted.stderr)
})

for (const filePath of ["Demo.nvue", "src/pages/demo.nvue"]) {
    test(`nvue 完整 TS 脚本和 Prettier 文件例外：${filePath}`, async () => {
        const source = '<template><view>{{ count }}</view></template>\n<script setup lang="ts">\nimport { ref } from "vue";\nconst count = ref<number>(1);\nuni.showToast({ title: "Hello" });\n</script>\n'
        const options = { ...prettierConfig, ...prettierConfig.overrides.find((item) => item.files === "*.nvue").options, filepath: filePath }
...
test("nvue 错误脚本不会被静默忽略", async () => {
    const engine = await eslint("uniapp")
    const [result] = await engine.lintText('<template><view /></template>\n<script setup lang="ts">const count: = ;</script>', { filePath: "src/pages/broken.nvue" })
    assert.ok(result.fatalErrorCount > 0)
})
...
function commitProject(t, style = "scss") {
    ...
    const run = (command, args) => spawnSync(command, args, {
        cwd: dir, encoding: "utf8", timeout: 60000, env,
    })
    function git(...args) {
        const result = run("git", args)
    ...
    write("package.json", JSON.stringify({ name: "full-hook-fixture", devDependencies: {
        eslint: "^9", "@dcloudio/uni-app": "3.0.0", [style === "less" ? "less" : "sass"]: "*",
    } }))
    let result = run(process.execPath, [path.join(root, "bin/init")])
    assert.equal(result.status, 0, result.stderr)
    result = run(process.execPath, [path.join(path.dirname(localRequire.resolve("husky")), "bin.js")])
    assert.equal(result.status, 0, result.stderr)
    return {
        dir, git, write,
...
for (const style of ["scss", "less"]) {
    test(`完整提交链：JS/TS/Vue/nvue/CSS/${style} 自动修复及二次复检`, (t) => {
        ...
        const samples = {
            ...
            "src/Demo.vue": '<template><view>Hello</view></template><script setup lang="ts">const count:number=1;console.log(count);</script><style>.demo{color:red;}</style>',
            "src/pages/Demo.nvue": '<template><view>Hello</view></template><script setup lang="ts">uni.showToast({title:"Hello"});</script><style>.demo{color:red;}</style>',
            "src/styles/main.css": ".demo{color:red;}\n",
            [`src/styles/main.${style}`]: style === "less" ? "@color:red;.demo{color:@color;}\n" : "$color:red;.demo{color:$color;}\n",
        }
...
for (const [name, source] of [
    ...
    ["src/Bad.vue", '<template><view /></template><script setup lang="ts">const value: = ;</script>'],
    ["src/Bad.nvue", '<template><view /></template><script setup lang="ts">const value: = ;</script>'],
...
test("部分暂存：仅提交暂存内容，保留工作区修改和未跟踪文件", (t) => {
    ...
    const staged = 'export const first="staged";\n' + "// unchanged context\n".repeat(12) + 'export const last = "base"\n'
    p.write(name, staged)
    p.git("add", "--", name)
...
test("部分暂存与格式化发生冲突时安全退出，不丢失原始修改", (t) => {
    ...
    const stagedSource = 'export const first="staged";\n' + "\n".repeat(12) + 'export const last="base";\n'
...
test("所有支持的文件只匹配一个任务组，组件工具严格按序执行", (t) => {
    ...
    for (const style of ["scss", "less"]) {
        ...
        for (const ext of ["vue", "nvue", "js", "ts", "jsx", "tsx", "cjs", "mjs", "mts", "cts", "html", "css", style, "json", "json5", "md", "yml", "yaml"]) {
            const matches = Object.keys(tasks).filter((pattern) => micromatch.isMatch(`src/中文 空格/file.${ext}`, pattern))
            assert.equal(matches.length, 1, `${style}/${ext}: ${matches}`)
        }
        assert.deepEqual(tasks["**/*.{vue,nvue}"], ["prettier --write", "eslint --fix", "stylelint --fix"])
    }
})
...
for (const ext of ["vue", "nvue"]) {
    test(`完整提交链拦截 ${ext} 内嵌样式错误（不是仅检查脚本）`, (t) => {
        ...
        const source = '<template><view>Hello</view></template>\n<style>\n.demo {\n    unknown-property: red;\n}\n</style>\n'
        p.write(name, source)
        p.git("add", "--", name)
        const head = p.git("rev-parse", "HEAD")
        const staged = p.git("diff", "--cached", "--binary")
