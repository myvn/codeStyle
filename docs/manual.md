...
| CHANGELOG 靠人写   | 要么漏写，要么事后考古                                | `standard-version` 按 feat/fix/perf 自动生成                          |
...
| 配置写错没人发现   | 规则冲突（如 `quotes` 与 Prettier 打架）长期潜伏      | 有 177 项自动化回归（含 ESLint 8/9/10、Stylelint 16/17 真实运行）守护 |
