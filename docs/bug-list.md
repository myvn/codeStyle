...
第十一轮（1.8.5 后·真实下游升级报告驱动）：下游 `bk-speedy-travel-v7-h5`（Vue 3 + Vite + pnpm）从 1.2.0 升级 1.8.5 的实测缺陷报告逐项核实，报告的 3 个上游缺陷全部证实并修复；测试基线增至 **189 项（基础 88 / 集成 65 / legacy 27 / Stylelint 17 共 9），全绿**。

| 编号    | 严重度 | 问题                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 修复                                                                                                                                                                                         | 回归测试                                                                                                   |
| ------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| BUG-031 | P1     | 统一/Less stylelint 配置把 recess-order 与 stylelint-prettier 全局启用：`.vue`/`.html` 的内联 `style="..."` 属性被 stylelint 侧重排/格式化，与 eslint-plugin-prettier（prettier 的 vue 解析器）互搏；且 postcss-html 对属性内容 round-trip 有损（实测引入前导空格、丢失分号后空格）→ lint-staged 链尾的 stylelint --fix 每次把提交产物改回 prettier-red 形态，门禁无法收敛（下游 9 个文件永久红）。修复过程中另踩一坑：recess-order 7 是 ESM 转译包，CJS `require` 拿到 `{__esModule, default}` 包装，直接取 `.rules` 得 undefined | recess-order 收进纯样式文件的 overrides（css/scss/less），用 override 级 `extends` 惰性解析——配置本体保持无 peer 可加载；vue/html 显式 `"prettier/prettier": null`，交给 prettier 全权格式化 | SL16 + SL17 双线各 2 用例：内联属性收敛（双向运行到同一不动点）+ recess-order 范围守护（纯样式文件仍生效） |
| BUG-032 | P2     | init 的 scripts 合并规则"旧值永远赢"，遗留的 `release: standard-version` 原样保留——standard-version 已停止维护且不在 peer 里，`pnpm release` 必然 command not found（下游实测）                                                                                                                                                                                                                                                                                                                                                    | `modifyPackageJson` 对该键唯一纠偏为 `commit-and-tag-version`，其余自定义脚本仍不覆盖                                                                                                        | init-matrix：纠偏生效 + 自定义 lint 脚本保留                                                               |
| BUG-033 | P3     | init 生成 `.versionrc.cjs` 后不清理同名旧配置：`.versionrc`/`.versionrc.json` 按发布工具查找顺序直接压过生成物，ESM 项目的 `.versionrc.js` 并存徒增"哪份在生效"的歧义（下游人工 `git rm` 收场）                                                                                                                                                                                                                                                                                                                                    | `--backup` 时把 stale 文件备份进 `.my-code-style-backup/` 并移除；无 `--backup` 保持告警                                                                                                     | init-matrix：ESM 项目双 stale 文件备份 + 移除 + 生成物接管                                                 |

报告核实备注（F 系列 = 流程/文档摩擦点）：

- **F1「无安装后体检」与实际不符**：init 第 5 步已有依赖体检——逐项列出缺失依赖 + 可整段粘贴的一键安装命令（eslint 10 项目实测只列真实缺失的 14 项，不含 legacy 专用的 classic import）。真实缺口是"仅打印、无强制"：`--install` / `doctor` 命令、lockfile 入库检查（F3）、hook 自检（F2）、`--merge` 模式（F5）记为增强候选，本轮未实施。
- **F4**：catv 钉 `^12` 的原因（13.x writer 回归，见第七轮 BUG-024 校准）已补进 README 发版章节。
- **O3**（stylelint 17 CLI 在 exit 2 时 `--formatter json` 的 stdout 偶发为空）：上游行为，留档备查。
...
第十四轮（1.9.0 下游回归反馈驱动）：修复 BUG-035 / BUG-036；基线增至 **197 项（基础 96 / 集成 65 / legacy 27 / Stylelint 17 共 9），全绿**。

| 编号    | 严重度 | 问题                                                                                                                                                                                                                                                                         | 修复                                                                                                                                                                                 | 回归测试                                                                                                         |
| ------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| BUG-035 | P1     | doctor 把 ESM-only peer 误报为未安装：stylelint-config-recommended@18 / recommended-scss@17 的 exports 只有 `"." : {"import"}` 且不暴露 `./package.json`，CJS require/resolve 双双抛 ERR_PACKAGE_PATH_NOT_EXPORTED，被当成未安装（下游正常项目 doctor 恒红，无法做 CI 门禁） | `installedVersion()` 改为文件系统链优先：从项目根与脚本位置逐级向上直读 `node_modules/<name>/package.json`（不受 exports 限制、符号链接可达兼容 pnpm），模块解析降为 exotic 布局回退 | doctor 识别 ESM-only peer 用例（内存构造 fake 包，零网络）；另以 registry 1.9.0+真实 18/17 装机复现并回验 exit 0 |
| BUG-036 | P2     | `--merge` 预览统计把将要保留的文件显示为「覆盖 9 个」，写入段才改口「保留」——行为正确但语义误导（下游实测）                                                                                                                                                                  | 预览逐项、小计、完成统计三处统一用 `保留/覆盖` 动词（mergeMode 分支），写入段去掉重复的"保留"提示                                                                                    | --merge 预览/完成统计语义断言 + 内容不被覆盖                                                                     |

集成说明（下游反馈，非 bug）：`pnpm lint` 只覆盖 ESLint，Stylelint 仅经 lint-staged 对改动文件执行——README 新增「lint 覆盖范围须知」；init 对有 CSS 预处理器的项目新增 `lint:style` 全量样式检查脚本（无预处理器不生成），补齐 CI 覆盖面。

第十五轮（1.9.1 下游 fresh init 回归驱动）：修复 BUG-037；基线增至 **200 项（基础 98 / 集成 66 / legacy 27 / Stylelint 17 共 9），全绿**。

| 编号    | 严重度 | 问题                                                                                                                                                                                                                  | 修复                                                                                                                                                                                                                                                                                                                            | 回归测试                                                               |
| ------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| BUG-037 | P2     | fresh init 生成的 flat 配置对显式 `.ts` 扩展名 import（`@/types/Foo.ts`，Vite bundler resolution 下合法）报 import-x/extensions 错——下游实测 **217 个错误**，删除旧配置后 fresh init 直接红一片（"绿起点"承诺被打破） | init 新增 `detectExplicitTsImports()`（扫描 src 等目录、上限 2000 文件）：命中则生成的 `eslint.config.mjs` 注入兼容段（该规则降级为 warn：带/不带扩展名均可写、不阻断 lint 退出码，风格不一致仍以警告保留渐进统一信号）并输出提示；干净项目保持严格默认。比 `--compat` 旗子自动化、比全局关闭保守（存量用户与新项目均不受影响） | init-matrix 注入/不注入两断言 + 集成 fixture 真跑（fresh lint exit 0） |

一并采纳的下游建议：

- **lint:all**：init 对有 CSS 预处理器的项目生成 `lint:all`（eslint + stylelint 全仓串联）；README 覆盖范围改为矩阵（lint / lint:style / lint:all / lint-staged / CI）。
- 217 处存量 import **不建议批量迁移**（下游判断一致）：兼容段（warn 降级）是长期共存方案，带/不带扩展名均可写，迁移时机的决定权在项目侧。
- 暂不做 `--compat` 旗子：自动检测已覆盖其场景，少一个要文档化的开关。

第十六轮（1.9.2 下游两套隔离验证反馈驱动）：修复 BUG-038；基线维持 **200 项，全绿**。

| 编号    | 严重度 | 问题                                                                                                                                                                                                                                                                                        | 修复                                                                                                                                                         | 回归测试                                                                                                                |
| ------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| BUG-038 | P1     | BUG-037 兼容段 `files` 只列 `ts/tsx/mts/cts`，漏 `.vue`/`.nvue`——`.vue` SFC 内的显式扩展名 import 绕过兼容段仍按 **error** 报。下游实测 165 errors + 52 warnings 全为 import-x/extensions：errors 全在 .vue、warnings 全在 .ts，分布与该缺口完全吻合                                        | 兼容段 files 补齐 `vue`/`nvue`：SFC `<script setup>` 内的显式 import 同样走 warn 口径。检测器 `exts` 本就含 `.vue`（注入判定无缺口），缺口仅在注入段的 files | init-matrix 断言 files 含 vue,nvue + 集成 fixture 增 `App.vue`（`<script setup>` 显式 import）真跑 lint exit 0 无 error |
| BUG-039 | P1     | `typescript` peer 钉 `^5.0.0`，把 Vite 8 模板默认的 TypeScript 6.0.3 项目堵死：npm 装机直接 ERESOLVE（唯一冲突源——typescript-eslint 8.70.1 自身接受 `>=4.8.4 <6.1.0`）、doctor 报红 exit 1。TS 6.0.3 真装实测（ESLint 10.11.0 组合）：init / eslint（0 errors）/ 兼容段全部正常，功能零障碍 | peer 放宽为 `^5.0.0 \|\| ^6.0.0`：与 typescript-eslint 现行支持上限对齐（TS 7 仍被两侧同时拒绝）。packed 包真装实测：ERESOLVE 消除、doctor TS 项不再报红     | config-contract 断言 peer 值 + init-matrix 缺失提示正则同步 + ts6 fixture 真装回验                                      |
